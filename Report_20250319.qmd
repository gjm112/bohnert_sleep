...
rbind(
dat %>% filter(!is.na(gender.v1)) %>% group_by(gender.v1) %>% summarize(
  n = n(),
  mean = mean(act_onset.time.min_sd, na.rm = TRUE)/60,
  sd = sd(act_onset.time.min_sd, na.rm = TRUE)/60
) %>% mutate(mean = paste0(mean %/% 1,
      ":",
...
dat %>% filter(!is.na(gender.v1))  %>% summarize(
  gender.v1 = "All", 
  n = n(),
  mean = mean(act_onset.time.min_sd, na.rm = TRUE)/60,
  sd = sd(act_onset.time.min_sd, na.rm = TRUE)/60
      ...
      sprintf("%02d", (round(60 * (
        sd %% 1
      ))))))  ) %>% write.csv("./table1_onset_sd.csv", row.names = FALSE)

rbind(
dat %>% filter(!is.na(gender.v1)) %>% group_by(gender.v1) %>% summarize(
  n = n(),
  mean = mean(act_outbed.time.min_sd, na.rm = TRUE)/60,
  sd = sd(act_outbed.time.min_sd, na.rm = TRUE)/60
) %>% mutate(mean = paste0(mean %/% 1,
      ":",
      sprintf("%02d", (round(60 * (
        mean %% 1
...
dat %>% filter(!is.na(gender.v1))  %>% summarize(
  ...
  sd = sd(act_outbed.time.min_sd, na.rm = TRUE)/60
      ...
      sprintf("%02d", (round(60 * (
        sd %% 1
      ))))))  ) %>% write.csv("./table1_outbed_sd.csv", row.names = FALSE)
...
datlong %>% group_by(I_frisat, ID) %>% summarize(
  n = n(),
  act_totsleep.min = mean(act_totsleep.min, na.rm = TRUE),
  act_efficiency = mean(act_efficiency, na.rm = TRUE),
  act_awakening.min = mean(act_awakening.min, na.rm = TRUE),
  ...
  summarize(gender.v1 = "All",
            ...
            act_outbed.time_numeric = paste0(
              ...
              ")"
            )) %>% select(gender.v1, I_frisat,n,act_totsleep.min, act_efficiency,act_awakening.min,act_onset.time_numeric,act_outbed.time_numeric),
...
Table 1 stuff ith trhe sd varabiles.  Sleep onset and sleep offset. 
```
...
library(GGally)
vif(mod3_totsleep)
datlong  %>% select(z_ED_EC_nat:z_SE_WL_nat ) %>% ggcorr(label = TRUE)
```
